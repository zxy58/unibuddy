      ...
      {tabs.map((tab) => {
        ...
        return (
          ...
          >
            ...
            <div style={{ width: 40, height: 28, borderRadius: 9, background: isActive ? '#FFF5F5' : 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'background 0.15s' }}>
              <span style={{ fontSize: 17, color: isActive ? '#ED1C24' : 'var(--text-tertiary)', transition: 'color 0.15s', lineHeight: 1 }}>
                {isActive ? tab.activeIcon : tab.icon}
              </span>
            </div>
            <span style={{ fontSize: 10, color: isActive ? '#ED1C24' : 'var(--text-tertiary)', fontWeight: isActive ? 700 : 400 }}>
              {tab.label}
            </span>
          </button>
        )
